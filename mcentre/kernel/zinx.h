...
#define GET_REF2DATA(type, ref, orig)         \
	type *pref = dynamic_cast<type *>(&orig); \
	if (nullptr == pref)                      \
	{                                         \
		return nullptr;                       \
	}                                         \
	type &ref = dynamic_cast<type &>(orig)
...
public:
	...
	{
		//pthread_mutex_lock(&mutex);
		auto ret = m_WriteBuffer.front();
		m_WriteBuffer.pop_front();
		//pthread_mutex_unlock(&mutex);
		return ret;
	}

	void Push(std::string &_szData)
	{
		//pthread_mutex_lock(&mutex);
		m_WriteBuffer.push_back(_szData);
		//pthread_mutex_unlock(&mutex);
	}
...
class ZinxKernel
{
	...
	static void Zinx_ClearChannelIn(Ichannel &_oChannel);
	friend class Ichannel;
	friend class ZinxTCPListen;
	friend class task;
...
public:
	...
	/*redis hget*/
	static std::vector<std::string> Zinx_Hget(std::string key, std::vector<std::string> fields);
	static std::vector<std::string> Zinx_HgetAll(std::string key);
	/*redis hset*/
	static bool Zinx_Hset(std::string key, std::map<std::string, std::string> fields);

private:
	ZinxKernel();
	~ZinxKernel();
	std::vector<std::string> HGET(std::string key, std::vector<std::string> fields);
	std::vector<std::string> HGET(std::string key);
	bool HSET(std::string key, std::map<std::string, std::string> fields);
	...
	struct epoll_event atmpEvent[MAX_THREADS];
